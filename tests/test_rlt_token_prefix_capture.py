...
class _FakeSmolVLA:
    def __init__(self, prefix: torch.Tensor | None) -> None:
        self.model = SimpleNamespace(vlm_with_expert=_FakePaligemmaWithExpert(prefix))

    def forward(self, batch, reduction: str = "mean"):
        self.model.vlm_with_expert.forward(inputs_embeds=[torch.empty(1, 1, 1), None])
        return torch.tensor(1.0), {"loss": 1.0, "reduction": reduction}
...
def _policy_with_fake_smolvla(prefix: torch.Tensor | None) -> RLTokenPolicy:
    policy = RLTokenPolicy.__new__(RLTokenPolicy)
    object.__setattr__(policy, "_pi05", _FakeSmolVLA(prefix))
    return policy
...
def test_forward_smolvla_with_prefix_captures_hidden_states_and_restores_forward():
    expected = torch.randn(2, 4, 8)
    policy = _policy_with_fake_smolvla(expected)
    target = policy._pi05.model.vlm_with_expert
    original_forward = target.forward
