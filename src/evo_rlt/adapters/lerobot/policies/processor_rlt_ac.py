...
class _BypassOnChunkTransitionPipeline(PolicyProcessorPipeline[dict[str, Any], dict[str, Any]]):
    ...
    ChunkTransitionDataset emits dicts already encoded through VLA + RL Token;
    those dicts lack the raw observation keys the VLA steps expect. The
    ``state_vec`` sentinel lets training bypass every step without losing the
    saved VLA JSON that ``lerobot-record`` reloads at deploy. At deploy,
    ``PolicyProcessorPipeline.from_pretrained`` returns a vanilla pipeline (this
    subclass is not preserved), so the full VLA step list runs.
    """
...
def make_rlt_ac_pre_post_processors(
    ...
    The saved preprocessor JSON is byte-identical to the SFT VLA's (modulo
    ...
    """
    del dataset_stats
    vla_pre, vla_post = load_sft_vla_processors(config.vla_pretrained_path, config.tokenizer_path)
    bypass_pre = _BypassOnChunkTransitionPipeline(
        steps=list(vla_pre.steps),
        name=POLICY_PREPROCESSOR_DEFAULT_NAME,
    )
    return bypass_pre, vla_post
